...
func (l *Ledger) ListZones(ctx context.Context) ([]Zone, error) {
	...
	for rows.Next() {
		var z Zone
		if err := rows.Scan(&z.ID, &z.Name, &z.Status, &z.UpdatedAt); err != nil {
			return nil, err
		}
		out = append(out, z)
	}
...
func (l *Ledger) CreateTransfer(ctx context.Context, in CreateTransferInput) (*Transaction, *string, error) {
	...
	if err := l.ensureAccount(ctx, tx, in.FromAccount, in.ZoneID); err != nil {
		return nil, nil, err
	}
	if err := l.ensureAccount(ctx, tx, in.ToAccount, in.ZoneID); err != nil {
		return nil, nil, err
	}
...
func (l *Ledger) ListRecentIncidents(ctx context.Context, limit int) ([]Incident, error) {
  ...
  `, limit)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&inc.ID, &inc.ZoneID, &related, &inc.Severity, &inc.Status, &inc.Title, &detailsBytes, &inc.DetectedAt); err != nil {
			return nil, err
		}
...
func (l *Ledger) Snapshot(ctx context.Context) (map[string]any, error) {
	...
	for rows.Next() {
		...
		var sp bool
		var ua time.Time
		if err := rows.Scan(&zid, &wb, &thr, &sp, &ua); err != nil {
			return nil, err
		}
  ...
  `)
	...
	for abRows.Next() {
		var id, zid string
		var bal int64
		if err := abRows.Scan(&id, &zid, &bal); err != nil {
			return nil, err
		}
  ...
  `)
	...
	for incRows.Next() {
		...
		var detailsBytes []byte
		var dt time.Time
		if err := incRows.Scan(&id, &zid, &related, &sev, &st, &title, &detailsBytes, &dt); err != nil {
			return nil, err
		}
		var d any
  ...
  `)
	...
	for spRows.Next() {
		...
		var ca, ua time.Time
		var aa *time.Time
		if err := spRows.Scan(&id, &req, &ph, &from, &to, &amt, &zid, &meta, &st, &fail, &ca, &ua, &aa); err != nil {
			return nil, err
		}
		var m any
		...
		if aa != nil {
			item["applied_at"] = aa.UTC().Format(time.RFC3339Nano)
		}
  ...
  `)
	...
	for aRows.Next() {
		...
		var details []byte
		var ca time.Time
		if err := aRows.Scan(&id, &actor, &action, &tt, &tid, &reason, &details, &ca); err != nil {
			return nil, err
		}
		var d any
...
func (l *Ledger) Restore(ctx context.Context, snap map[string]any) error {
	...
	if al, ok := snap["audit_log"].([]any); ok {
		for _, it := range al {
			...
			if actor == "" || action == "" || tt == "" || tid == "" {
				continue
			}
			reasonAny := m["reason"]
			var reason *string
			if rs, ok := reasonAny.(string); ok && rs != "" {
				reason = &rs
			}
			details := m["details"]
...
func (l *Ledger) ListBalances(ctx context.Context, limit int) ([]BalanceRow, error) {
  ...
  `, limit)
	...
	for rows.Next() {
		var b BalanceRow
		if err := rows.Scan(&b.AccountID, &b.BalanceUnits, &b.UpdatedAt); err != nil {
			return nil, err
		}
		out = append(out, b)
	}
...
func (l *Ledger) ListTransactions(ctx context.Context, limit int) ([]TransactionRow, error) {
  ...
  `, limit)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.RequestID, &t.FromAccount, &t.ToAccount, &t.AmountUnits, &t.ZoneID, &t.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, t)
	}
...
func (l *Ledger) GetTransaction(ctx context.Context, id string) (*TransactionDetail, error) {
  ...
  `, id)
	...
	for rows.Next() {
		var p PostingRow
		if err := rows.Scan(&p.AccountID, &p.Direction, &p.AmountUnits); err != nil {
			return nil, err
		}
